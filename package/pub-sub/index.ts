...
class PubCenter extends User {
  // 触发订阅
  private $trigger = async (handlers: ListenerHandler[], ...args: any[]): Promise<HandleResult[]> => {
     ...
     const filterResult: Record<'origin' | 'trigger', ListenerHandler[]> = listeners[name].reduce((store: any, current) => { 
        if(!isType(current, 'Function')) {
          ...
          if(isType(query, 'String')) {
            ...
            (query as Id[]).includes(id) && store.trigger.push(current)
          } else {
            store.origin.push(current);
          }
        } else store.origin.push(current)
        return store
     }, { origin: [], trigger: [] });
  ...
  // 取消订阅   
  private $unListen = (name: string, handler?: HandlerFn | Id | Id[]) => {
    const listeners = this.getListener();
    ...
    listeners[name] = listeners[name]?.filter?.(fnItem => {
      if(isType(handler, 'Funtion')) {
        return isType(fnItem, 'Funtion') ? fnItem !== handler : (fnItem as HanlderConfig).handler  !== handler;
      } else {
